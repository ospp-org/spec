...
**Date:** 2026-09-10
**Specification-document version:** 0.43.0 (release tag `v0.43.0`)
**Status:** 3 blockers open (all BLE), **27** non-blocking issues open, **25** decisions recorded (one of
...
| 17 | **A TriggerMessage refusal naming its code** | no **field**: `trigger-message-response` carried `status` closed at `Accepted`/`Rejected`/`NotImplemented` and nothing else, under `additionalProperties: false` — and the *Implicit Error Codes* note makes `1005`, `2007` and `6001` implicit for **every** Server → Station REQUEST, so three codes were mandated onto a message with nowhere to put one | **CLOSED 0.34.0** — `errorCode` and `errorText` added, **REQUIRED when `status` is `Rejected`**. Station → server, the direction in which widening a response cannot break a validator |
...
rule addressed to an actor the artefact does not admit (9); and **three** are a rule that requires a
truthful value from a **closed enumeration in which no legal value is true** (11, 12, 13).
The four sum: 11 + 3 + 1 + 3 = 18.

**Instances 17 and 18 are the same defect on two of fourteen actions**, the way 15 and 16 were the
same defect on two halves of one session — and they were found the same way, by reading a rule (the
*Implicit Error Codes* note) against the schemas that would have to carry what it mandates. The
denominator is what made them findable: not *"does this response look thin?"* but *"how many of the
14 Server → Station REQUEST actions cannot express any of the three codes the note makes implicit for
all of them?"* The answer was two, and once the question was asked in that form neither could hide.
**18 supplies no fifth remedy**, but it does sharpen the third: the widening it needed turned out to
be smaller than it looked, because one member the shape seemed to require relaxing already carried a
truthful value on the refusal branch. Ask what the existing value means before deciding a shape has
no room.
...
## DECIDED (0.40.0) — one field carried three different length limits, and the split was not the one its two meanings draw
...
`signature` were checked against this shape and do NOT match it — zero descriptions on either
carry more than one limit — so this closes as a single-field defect rather than the three-name
...
## CLOSED — `5016 VERSION_ALREADY_INSTALLED` carried two conditions, and option 2 was taken: one predicate, one code

**Raised 2026-08-17, in the firmware-cycle arc. Closed 2026-09-12.**

**Resolution: option 2, broadened — with the discriminator that option 2 said it would need turning
out to be unnecessary.** The predicate is now the **union**: a station refuses an offered
`firmwareVersion` that is **the same as or older than** the installed one, and installs only a
strictly newer one ([`06-security.md` §4.6.1](spec/06-security.md#461-anti-downgrade-protection)).
All **6** sites that stated a predicate now state the union — the 4 that said *equality*
(`07-errors.md` §3, `update-firmware.md` §5 rule 6, that file's §9 table, `03-messages.md` §6.4)
and the 2 that said *older than* (`06-security.md` §4.6.1, `implementors-guide.md`).

**Why no `details` discriminator was added after all.** Option 2 assumed the two recoveries stay
distinguishable only if the code carries which condition fired. They do not need to be: under the
union the operator recovery is **the same one** — offer a strictly newer version, or set
`forceDowngrade` for a deliberate re-install or rollback. The Recommended Action that was *"actively
wrong for the downgrade case"* was wrong because it assumed the server's records were stale; the
row now states the condition rather than a diagnosis, so it is correct for both halves.

**And the name is no longer false half the time.** `VERSION_ALREADY_INSTALLED` under the union
reads as *"the version you offered is not ahead of what is installed"*, which covers equality
exactly and covers older-than without claiming the station ran it. Option 1 (`5013
DOWNGRADE_REFUSED`) was costed and not taken: a new code is a MINOR every implementation must
learn, and it buys a distinction no recovery depends on.

**What was fixed alongside, and it was the larger half.** *"Older than"* had no comparison rule at
all. It is now numeric on the three components, with string comparison forbidden — `1.10.0` ordered
before `1.9.0` by a string comparison is a `FirmwareDowngradeAttempt` raised against a legitimate
rollout, at a point in a product's life that is certain.

**Radius: 0 schema bytes, 0 conformance vectors, 0 example payloads, 0 new codes.**
...
*Related but distinct:* [`5016` is required for two conditions](#closed--5016-version_already_installed-carried-two-conditions-and-option-2-was-taken-one-predicate-one-code)
