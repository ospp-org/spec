...
**Date:** 2026-08-18
**Specification-document version:** 0.24.0 (release tag `v0.23.0`)
**Status:** 3 blockers open (all BLE), 20 non-blocking issues open, 11 decisions recorded (one of
...
## OPEN — `OfflinePassMaxAge` mandates a refusal that no validation list performs

**Raised 2026-08-18, in the offline spec-contradiction cycle. Recorded rather than repaired: both
ways out are defensible and they differ in whether a capability is kept.**

[`spec/08-configuration.md` §5](spec/08-configuration.md) defines `OfflinePassMaxAge` — integer,
default `3600`, range `300--86400` — as *"Maximum age in seconds for an OfflinePass to be
considered valid. Passes older than this value MUST be rejected."*

Nothing performs that rejection. It is not among the station's checks
([`offline-pass.md` §4](spec/profiles/offline/offline-pass.md),
[`06-security.md` §6.1.1](spec/06-security.md)), not among the eleven at authorize-time
([`authorize-offline-pass.md` §5](spec/profiles/offline/authorize-offline-pass.md)), and not among
the thirteen at reconcile ([`reconciliation.md` §6.1](spec/profiles/offline/reconciliation.md)).
The only temporal check any of them makes is on `expiresAt`. Measured across all five
implementation trees at this HEAD: the key is present in two SDK configuration registries with its
`3600` default and has **no reader anywhere**; no `issuedAt`-based age comparison exists.

**The default is the sharp end.** A pass may be signed for up to 24 hours
(`offline-pass.md` §6 step 1, a **MUST NOT** on the issuer), so a station honouring
`OfflinePassMaxAge` at its default would refuse a pass after **1/24 of the validity the server
signed into it** — while check #2 reads the same pass as valid.

**What would close it.** Either (a) the key is **withdrawn**, as `DiagnosticsUploadUrl` was in
0.23.0 for having no reachable consumer — which removes a real capability, since the key is the
only way an operator can shorten effective pass life fleet-wide without re-issuing passes; or
(b) the check is **added** as a station-side #11, with a default that does not contradict the 24-hour
issuance cap. Adding it at the current default would make a legitimate operation impossible, which
is the worse of the two failure modes; adding it at `86400` makes it inert by default and available
when configured. Note that (b) changes a cited count — "10 checks" is named in three documents and
in the profile index — and an ordinal is an identifier.
