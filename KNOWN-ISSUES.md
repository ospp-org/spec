...
**Date:** 2026-09-03
**Specification-document version:** 0.30.0 (release tag `v0.30.0`)
**Status:** 3 blockers open (all BLE), **30** non-blocking issues open, **21** decisions recorded (one of
them reversing another), and one named defect **class** with **fifteen** instances. **The counts are
...
## OPEN (0.30.0) — seven gaps whose only repair is a schema byte, and why not one of them was taken here

**This release moves ZERO bytes under `schemas/`.** That is not an accident of scope: it is the
fifth, sixth and seventh time the same trade has been refused, on the reason measured once and not
re-litigated since — *an added member is backward-compatible for the emitter and not for a receiver
validating against an older vendored copy.* Both SDKs vendor all 86 schemas **byte-identically** and
gate on `diff -rq`, so a single byte turns both CIs red until a re-vendor, a re-tag, and a server pin
bump. The seven are recorded here with their option space so the cost is visible when someone does
decide to pay it.

| # | Gap | What cannot carry it | The schema edit it wants | Cheapest thing available without it |
|:--:|---|---|---|---|
| 1 | **No `bootReason` names a Boot requested by TriggerMessage** | the **enum**: 8 members, of which 7 name an actual boot and the 8th (`Reconnect`) is defined as *"the firmware never restarted, only the MQTT session is new"* — untrue here, since the session is not new either | a 9th member on `boot-notification-request` | nothing honest. Mandating `Reconnect` makes that member's own definition false in three documents. **Note the hidden cost:** both SDKs hard-code the derived helper as `!== RECONNECT`, which a 9th non-boot member silently falsifies |
| 2 | **`SessionTimeout` has no `reason` value** | the **enum**: `reason` is closed at 6 and none is inactivity. The chapter already concedes it — *"widening the enum is a coordinated wire change and has deliberately **not** been made"* | a 7th member on `session-ended-event` | the other two thirds of the note *are* repairable in prose (the §3.4-vs-registry trigger disagreement, and the two broken settings) and remain so. **Hard break at the far end:** the server's transformer `tryFrom()`s the enum and **throws** on an unknown value, and its handler matches all six with no default arm |
| 3 | **No boot generation on the wire** | no field. `sessionGeneration`, `bootGeneration`, `authority_epoch` are **0 occurrences** in every repository | a REQUIRED integer on `boot-notification-request`, and on the envelope if it is to ride many messages | **a boot epoch is already derivable and the server already derives one** — `uptimeSeconds` against the server clock — and the specification already requires the `bootReason`/`uptimeSeconds` pair to agree. Prose can make that cross-check a server **MUST** and require refusing an inconsistent pair. What prose cannot buy is *authority*: every current input is the station's own assertion at second granularity. **Widest cascade of the seven** — if it touches the envelope, all 334 conformance vectors carrying one become invalid, and no CI gate diffs that corpus |
| 4 | **`MessageSigningMode` has no wire signal** | the **schema**: `capabilities` has 4 boolean keys with `additionalProperties: false`, and the root object is closed too, so a fifth is forbidden at both levels | a 5th capability key | **none, and this is the sharp one.** The mode *is* readable — it is registry key #18, via GetConfiguration — but GetConfiguration is among the 44 signed message types, so the only channel that can read the mode is the channel a mode mismatch closes. The one message that always arrives is BootNotification REQUEST, which is closed at both levels. A fail-closed station is silent by construction |
| 5 | **`SecurityEvent.details` is REQUIRED with no shape** | no constraint: `{"type":"object","additionalProperties":true}`, so `{}` validates | an `if`/`then` on `type`, or at minimum `minProperties: 1` | prose alone would be a fourth unenforced restatement, and the cost is already measured in this file — the `FirmwareDowngradeAttempt` event names the offered version under two different member names and nothing can tell |
| 6 | **`UpdateServiceCatalog.services` has no `maxItems`** | no bound, against a 64 KB packet ceiling. 8 of 86 schemas do carry `maxItems`, so the keyword is in the vocabulary | `maxItems` on `update-service-catalog-request` | the **prose half is repaired at 0.30.0**: `5025`'s Recommended Action told servers to *"check station capabilities for maximum catalog size"* — a field that has never existed — and now names the 64 KB ceiling instead. *(The claim's other half is false: `certificate-install-request` contains **zero** arrays; both members are `maxLength`-bounded strings.)* |
| 7 | **No confirmation that a stop physically stopped** | the **schema**: `stop-service-response` is closed over 7 members and has never had `actuator` or `stopConfirmed`; 0 occurrences across all 47 MQTT schemas | a new member, on a closed **RESPONSE** — the exact rejected shape | **repaired in prose at 0.30.0 as far as prose reaches**: rule 9's bare SHOULD becomes a **MUST** to report through two messages that already exist — a `HardwareFault` SecurityEvent whose `details` object is open and must carry the `sessionId`, and a `Faulted` StatusNotification. What that does **not** buy is a value on the settling response itself; settlement still runs on the self-reported duration |

**Instances 1, 2, 4, 5 and 7 are new members of the defect class indexed below.**
...
## OPEN (0.30.0) — the provisioning response schema contradicts itself about whether `keepAliveSeconds` may be absent
...
**The schema half is not repaired, and the reason is the cost, not the difficulty.** Editing a
`description` string changes no validation whatsoever and still moves bytes under `schemas/`, which
turns both SDK byte-identity gates red until a full re-vendor and re-tag. A description that
contradicts its own `required` array is worth exactly one release of waiting for a cycle that is
paying that cost for a real reason. Whoever next opens `schemas/`, take this with it.
...
| 11 | A BootNotification **requested via TriggerMessage** reporting why it booted | the **enum member**: `bootReason` has 8 values, 7 name an actual boot and the 8th is defined as *"the firmware never restarted, only the MQTT session is new"* — and here the session is not new either. Not a corner case: `BootNotification` is one of only two triggers a **restricted** station may accept, i.e. the designed operator escape from `Pending` | [OPEN 0.30.0](#open-0300--seven-gaps-whose-only-repair-is-a-schema-byte-and-why-not-one-of-them-was-taken-here) |
| 12 | A session ended by **inactivity** reporting its reason | the **enum member**: `reason` is closed at 6 and none is inactivity. The chapter states the gap about itself — *"`SessionTimeout` is not fully specified, and this note is the specification of that fact"* — and declines the widening in the same breath | [OPEN 0.30.0](#open-0300--seven-gaps-whose-only-repair-is-a-schema-byte-and-why-not-one-of-them-was-taken-here) |
| 13 | A station **blocked fail-closed on a signing-mode mismatch** saying so | the **enum member**, and then the actor: `capabilities` carries 4 booleans under `additionalProperties: false` at both levels, so a fifth is forbidden — and the mode *is* readable by GetConfiguration, which is itself a signed message, so the only channel that can read it is the channel the mismatch closes | [OPEN 0.30.0](#open-0300--seven-gaps-whose-only-repair-is-a-schema-byte-and-why-not-one-of-them-was-taken-here) |
| 14 | A **SecurityEvent that can be triaged** | no **shape**: `details` is REQUIRED and typed `{"type":"object","additionalProperties":true}`, so `{}` satisfies the requirement. The cost is already recorded in this file — `FirmwareDowngradeAttempt` names the offered version under two different member names and nothing can tell | [OPEN 0.30.0](#open-0300--seven-gaps-whose-only-repair-is-a-schema-byte-and-why-not-one-of-them-was-taken-here) |
...
rather than carried from a note; **nine** are still open and one is a blocker. There are now **four**
...
enumeration in which no legal value is true** (11, 12, 13).
