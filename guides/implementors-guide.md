...
6. Compare the offered `firmwareVersion` against the currently installed version, **as three integers, numerically — never as strings** ([`06-security.md` §4.6.1](../spec/06-security.md#461-anti-downgrade-protection)). If the offered version is **the same as or older than** the installed one and `forceDowngrade` is not `true`, reject with `5016 VERSION_ALREADY_INSTALLED`. Log a `FirmwareDowngradeAttempt` SecurityEvent only where the offered version is **strictly older** — an equal version is a re-issue, not a downgrade attempt. Only a strictly newer version is installed.
...
*This guide covers OSPP 0.39.1. For normative requirements, always refer to the [spec chapters](../spec/). For message field definitions, refer to the [JSON Schemas](../schemas/). For realistic examples, see the [example payloads and flows](../examples/).*
