...
4. **BootNotification gates everything the station originates.** Until BootNotification is Accepted the station MUST NOT send StatusNotification, Heartbeat, or any other EVENT or REQUEST of its own — except SignCertificate while `Pending`, which renews the station's own certificate and so repairs its standing rather than reporting its work. This ensures the server has validated the station's identity and established the HMAC session key before processing any operational data. A **RESPONSE** is the exception, and only in `Pending`: that is the restricted state in which the station answers commands so an operator can repair whatever is holding the boot ([Chapter 05 §1.4](../../spec/05-state-machines.md#14-the-restricted-states)).
