...
- The **specification-document version** shown in each chapter header (e.g. *OSPP Version: 0.43.0*) versions this specification's prose and schemas. It is **independent of** the wire `protocolVersion` field carried in the message envelope (e.g. `0.3.0`): the two version numbers evolve separately and need not match.
...
>
> **What the comparison is made over, and why it is the `payload` and not the envelope.** Content equality is equality of the **OSPP Canonical Form** ([`06-security.md` §4.8](06-security.md#48-ospp-canonical-form)) of the **`payload`**, and of nothing else. The payload is the claim; the envelope around it is transport. So key order, whitespace and other insignificant encoding differences **MUST NOT** make two messages differ here — the payload sits inside the byte sequence [§5.4](06-security.md#54-mac-computation) already MACs, so a message whose `mac` verified has been reduced to this form already, and a comparison stricter than the one the signature survived would refuse frames the receiver has itself proved authentic.
>
> **Comparing the envelope would be a defect, not a stricter reading.** `timestamp` and `mac` legitimately differ between an original and a re-sent copy of the same claim: a sender that re-composes a REQUEST after a response timeout **MUST** keep the `messageId`, because that is what correlates the answer ([§3.2](#32-message-ordering)), and it will normally re-stamp the time. An envelope-level comparison calls every such retry a collision and rule 4 then processes it a second time — the exact double-execution rule 3 exists to prevent, arrived at through the rule meant to protect against losing a message. Two identical claims are one claim however the envelope around them was stamped.
>
...
- The LWT is **exempt from message signing** (the `mac` field is absent). It is one of the three structural exemptions enumerated in [Chapter 06 §5.6](06-security.md#56-message-signing-classification), which is the single source for what is exempt and why; the reason is not only that it is registered at CONNECT time, but that CONNECT precedes the BootNotification RESPONSE that issues the session key ([Chapter 06 §5.9](06-security.md#59-session-key-lifetime) rule 1) and the broker publishes the will after the station is gone — so its bytes are fixed when no key for that session exists yet, and sent when nobody is there to sign them.
