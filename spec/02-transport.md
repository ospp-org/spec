...
- **TransactionEvents**: The `txCounter` field provides strict ordering for offline transaction reconciliation (per-pass, per-station; see [`profiles/transaction/transaction-event.md`](profiles/transaction/transaction-event.md)).
- **Online session-scoped EVENTs (Per-Session `seqNo`, OPTIONAL)**: MeterValues and SessionEnded MAY carry a per-session monotonic `seqNo` field starting at `0` for the first session-scoped EVENT and incrementing by exactly `1` for each subsequent EVENT in the same session (same `sessionId`). If `seqNo` is present:
    ...
    - On detected gap (e.g., received seqNo `5` after seqNo `3`), the receiver SHOULD log a warning. If the missing seqNo range crosses a billing-milestone boundary — for example, the `< 50% duration delivered` threshold defined in the refund policy at [`04-flows.md §6`](04-flows.md) — the receiver MUST flag the session for HIGH-severity reconciliation audit. This mirrors the gap-handling rule for `txCounter` in [`profiles/transaction/transaction-event.md §7.1`](profiles/transaction/transaction-event.md).
...
The station-provisioning endpoint (`POST /api/v1/stations/provision`) implements this contract keyed on the **provisioning token itself** rather than an `Idempotency-Key` header — but the token alone is **not** the whole key. Within the token's 24-hour TTL, a repeat presenting the **same public keys** returns the originally-issued certificate, whereas a repeat presenting a **different** public key is **not** a duplicate and **MUST** be rejected with `409 Conflict` / `4015 PROVISIONING_KEY_MISMATCH` rather than replayed. After the TTL the token is invalid for all purposes. See [Flows §2 — Single-use and idempotent retry](04-flows.md#single-use-and-idempotent-retry).
