...
The station **SHOULD** reject a firmware update whose offered `firmwareVersion` is **the same as or older than** the currently installed version, and **MUST** install only a strictly newer one. A rejection on this ground **MUST** carry error `5016 VERSION_ALREADY_INSTALLED`.

**The SecurityEvent stays bound to the strictly-older half, and the split is deliberate.** A station **MUST** log a SecurityEvent [MSG-012] with `type: FirmwareDowngradeAttempt` when it refuses a **strictly older** version, and **MUST NOT** raise one when it refuses an **equal** one. The two halves share a refusal because they share a recovery; they do not share a threat. An equal version is a re-issue — an operator re-sending a command, a server retrying a rollout — and raising a security event on it would put a `Warning` on the operator's console for the most ordinary mistake there is, which is how a signal stops being read. A strictly older version offered to a station that has moved past it is the case the event exists for.

**One predicate, because one code cannot carry two.** `5016` was attached to *equality* in four places — this chapter's neighbour [`update-firmware.md` §5](profiles/device-management/update-firmware.md) rule 6, that file's §9 table, the [Chapter 07 §3](07-errors.md) registry row, and [Chapter 03 §6.4](03-messages.md) — and to *older than* in this one and in [`implementors-guide.md`](guides/implementors-guide.md), which is **4 of 6** against **2 of 6** for the same code — while a **7th** site, [Chapter 03 §6.4](03-messages.md)'s idempotency row, called a repeat a *no-op* and named no status at all. A receiver meeting a `5016` could not tell which condition produced it, and a station could satisfy either reading and be conforming. The union is the predicate that makes both readings true at once and loses nothing: *same* was already refused, *older* was already refused, and no third case existed. Every one of the five sites now states the union.

**"Older than" is a numeric comparison of three integers, and the schema has already reduced it to that.** `firmwareVersion` matches `^\d+\.\d+\.\d+$` ([`update-firmware-request.schema.json`](../schemas/mqtt/update-firmware-request.schema.json)) — three decimal components, no pre-release identifier and no build metadata — so a version **is** a triple, and the comparison is the ordinary left-to-right comparison of those three components **as numbers**. A station **MUST** compare them numerically and **MUST NOT** compare the strings. A string comparison orders `1.10.0` before `1.9.0`, so the first release whose minor or patch reaches two digits is read as a downgrade, refused with `5016`, and reported as a `FirmwareDowngradeAttempt` — a security alert raised against a legitimate rollout, at a point in a product's life that is certain rather than hypothetical. Leading zeros are admitted by the pattern and carry no meaning under this rule: `1.02.0` and `1.2.0` are the same version.

**What `forceDowngrade` still does, and it is unchanged by the union.** The flag bypasses this refusal, so an explicitly requested re-install of the running version and an explicitly requested rollback both proceed — that is the whole point of an operator-set flag, and widening the refused set widens what the flag is there to override. It does not bypass [§4.6](#46-firmware-code-signing) signature verification, which runs on every image regardless.
