...
2. The station **MUST** compute `actualDurationSeconds` from a monotonic timer, not the wall clock ([`heartbeat.md` §6](../core/heartbeat.md#6-clock-synchronization) rule 5), and **MUST** round the interval to the **nearest** second rather than truncating it — the same obligation the other carrier states, with its arithmetic, at [`stop-service.md` §5](stop-service.md) rule 5.
