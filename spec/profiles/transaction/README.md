...
- **ReserveBay**: a request for an already-reserved bay **MUST** return `3014 BAY_RESERVED` — *except the idempotent repeat*. A request whose `reservationId` names the reservation the bay currently holds, and whose payload is otherwise identical, **MUST** return the same `Accepted` response it returned the first time ([`reserve-bay.md` §5.1](reserve-bay.md) rule 7, with the retention regime that makes it answerable in §5.2). The two are not alternatives a receiver may choose between: rule 7 is evaluated first, and `3014` is what a **different** reservation gets. This bullet said only the second half until 0.30.0, while rule 7 and its four sibling restatements have said both since 0.14.0.
