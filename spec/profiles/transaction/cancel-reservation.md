...
3. If the reservation has already expired (auto-released by timer), the station **MUST** respond with `3013 RESERVATION_EXPIRED`. Answering this requires the station to still hold the record; the retention that makes it possible is [`reserve-bay.md` §5.2](reserve-bay.md).
...
5. The station **MUST** send a StatusNotification event after the bay transitions back to `Available`, within 1 second. This is a bay state change like any other and [CORE-005](../core/README.md) admits no exception for it. An earlier revision said **SHOULD** — the same defect already repaired in [`reserve-bay.md` §6](reserve-bay.md), and it fails the same way: a server left holding a bay `Reserved` after the station released it, with no conforming way to find out.
