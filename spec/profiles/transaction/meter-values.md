...
5. If the interval elapses but no meter readings have changed since the last event, the station **MAY** skip that interval — **and only where the running program is not expected to advance any member the station reports.** Where it is, the station **MUST** send the event: during an active delivery, a member that should be advancing and is not is the signature of a stalled pump, a closed valve or an empty consumable, and skipping suppresses the one report that would carry it. A station that cannot decide which members a program advances **MUST** send the event.

   **Why the permission is conditioned on the program rather than stated flat.** Which members move is a property of the program, not of the protocol: [`meter-values.schema.json`](../../../schemas/common/meter-values.schema.json) requires at least one of `liquidMl`, `consumableMl` and `energyWh` and binds none of them to any program, so a rinse advances `liquidMl` while a blow-dry legitimately advances none of the three on a station that meters only liquid. Read flat, the permission is widest exactly where it is least safe — the unchanged reading during a delivery that should be changing — and narrowest where it costs nothing. The station is the only party that knows which members its programs advance, and this rule binds it to act on that knowledge rather than on the bare comparison. The receiver cannot check it: nothing in this specification obliges a server to hold a program-to-member map, and [Chapter 02 §3.2](../../02-transport.md#32-message-ordering) verifies `seqNo` continuity only — which a skipped interval does not break, since `seqNo` counts events sent rather than intervals elapsed.
