...
| `messageSigningMode` | string | **No** | `"All"` or `"None"` — the mode the station has **just booted into**, from NVS. Added `0.31.0`. It is not a capability and is deliberately not inside `capabilities`: that object is feature flags describing what the station *supports*, this is configuration state describing what it is *doing*. It rides this message because BootNotification REQUEST is one of the three messages exempt from signing, and therefore the only one that still arrives when the two sides disagree about the mode — every other channel, GetConfiguration included, is refused by the very condition it would report ([Chapter 06 §5.7](../../06-security.md#57-failure-handling--both-directions-fail-closed)). OPTIONAL, so its **absence carries no meaning**: a server **MUST NOT** infer a mode from a boot that omits it. |
...
**It is the value for a second case too, and saying so is the whole of the repair.** TriggerMessage [MSG-016] can ask a station for a BootNotification ([`trigger-message.md`](trigger-message.md) — and it is one of only two triggers a **restricted** station may accept, so it is the designed operator escape from `Pending`). Nothing restarts: the station composes and sends the message on the connection it already has. Seven members name a boot that did not happen, and `Reconnect`'s definition said *only the MQTT session is new* — which is not true either, since the session is the one it was already on. **That left a triggered Boot with no truthful member at all**, which is precisely the bind `Reconnect` was invented to end, reappearing one case over. `Reconnect` therefore covers **both** cases of *no boot occurred*: a re-dialled session, and a Boot the server asked for. **No member is added**: the enum stays at eight, and a station already implementing `Reconnect` needs no new code path — what changes is that it may now use it honestly.
...
3. A server **MUST** cross-check the pair on every BootNotification, and **MUST NOT** act on a `bootReason` its `uptimeSeconds` contradicts. The inconsistent reports are a `PowerOn`, `Watchdog`, `FirmwareUpdate`, `RemoteReset`, `ManualReset`, `ScheduledReset` or `ErrorRecovery` arriving with an uptime that did not reset, and a `Reconnect` whose uptime went **backwards** against the value the same station last reported. On either, the server **MUST** take the branch it would take for a real boot — terminate and settle any session it was holding for that station — and **MUST** log the contradiction. Refusing the *claim* is the whole obligation: the BootNotification itself is still answered normally, because a station cannot repair this by retrying and closing the channel would strand it.

    **The limit, stated rather than left to be discovered.** This is a consistency check between two values the *same station* supplies, so it catches a station that contradicts itself and nothing else. **A station that reports a coherent falsehood is undetectable by it** — a `Reconnect` with a plausibly-advanced uptime is indistinguishable from the truth, and no rule in this specification can make it otherwise. What would close that is a boot generation the station persists and cannot understate, which is a new wire concept on many messages and is **not** taken here ([KNOWN-ISSUES](../../../KNOWN-ISSUES.md)). Read this rule as raising the cost of an inconsistent report from zero to detection, not as establishing authority over what the station asserts.
...
| Invalid message format | `1005 INVALID_MESSAGE_FORMAT` | Server responds **`Rejected`** carrying `errorCode`, `errorText` and `retryInterval`. [`boot-notification-response.schema.json`](../../../schemas/mqtt/boot-notification-response.schema.json) requires all three on **any** `Rejected` boot response and exempts no code, so a `1005` boot rejection that carried none would not validate. Station **MUST** keep retrying per CORE-011 — see the `1005` row in [Chapter 07 §3.1](../../07-errors.md#31-transport-errors-1xxx) for why *do not retry* there does not suspend it. **This row is the payload case, and it is the only case that can be one**: the envelope parsed, so the server knows the action and the `messageId` and can correlate a response. A message whose *envelope* cannot be parsed is discarded without a reply, but it is not a BootNotification failure at all — nothing identifies it as a BootNotification — and it is governed by [Chapter 02 §11](../../02-transport.md#11-error-scenarios-summary), which belongs to no profile. This row read *"Server drops the message. Station does not receive a response"* until 0.30.0, against the schema, against §2.1 of Chapter 07, and against a server that has answered on this path all along. |
...
| Declared `protocolVersion` is not in the server's supported set | `1007 PROTOCOL_VERSION_MISMATCH` | Negotiation is **exact match** — a shared MAJOR implies nothing ([VERSIONING.md](../../../VERSIONING.md)). Server responds with `Rejected`, and **MUST** include both the `supportedVersions` array and a `retryInterval`. Station **MUST** keep retrying BootNotification per CORE-011 and **MUST NOT** stop: it accepts no commands while rejected, so it cannot be handed a firmware update in that state, and the retry is also what recovers it if the server later adds its version. The cause is fixed by upgrading station firmware to a version in `supportedVersions`, or by widening the server's set, after which a retry succeeds. |
