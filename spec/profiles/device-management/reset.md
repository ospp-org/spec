...
2a. A server **MAY** decline to dispatch an unforced Reset it can already see the station would refuse — it holds the session state and knows the answer. A server that does so **MUST** answer `6008 COMMAND_PRE_EMPTED` with `details.wouldBe: 3016` and `details.reason` naming the condition, and **MUST NOT** answer `3016` itself: that code means the station received the command and declined, and an operator has to be able to tell the two apart, because a pre-empt on a stale server view is repaired by reconciling the server and not by visiting the station. This is the **predicted-refusal** kind of pre-empt, which is why `details.wouldBe` is present; the server-protective kind carries none ([`07-errors.md` §3.6](../../07-errors.md)). Pre-empting is an optimisation, never a requirement — dispatching and letting the station answer is equally conforming. A server **MUST NOT** pre-empt a Reset carrying `force: true`, however many sessions are running: forcing is the instruction to proceed anyway, so there is nothing to pre-empt.

**This rule binds an answer, not an interface.** OSPP defines no REST endpoint through which an operator commands a Reset, and [`07-errors.md` §2.4](../../07-errors.md) places administrative surfaces outside this specification, so **the pre-empt path here is not conformance-testable** and no test case exercises it. The obligation is still worth stating: a deployment that pre-empts and reports it as `3016` has told its operator the station refused, when the station never saw the command. What is unbound is the carrier, not the distinction.
