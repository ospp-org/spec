...
6. If the requested `firmwareVersion` is **the same as or older than** the version the station is running, it **MUST** respond with `Rejected` and error code `5016 VERSION_ALREADY_INSTALLED`, unless `forceDowngrade` is `true`. Versions are compared as three integers, numerically, never as strings ([Chapter 06 §4.6.1](../../06-security.md#461-anti-downgrade-protection)). Only a strictly newer version is installed.
...
| `5016` | `VERSION_ALREADY_INSTALLED` | Warning | The requested firmware version is the same as or older than the one the station is running, and `forceDowngrade` was not set. |
