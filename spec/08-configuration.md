...
| `StationIdentityCertificate` | string | -- | W | Dynamic | max 500 chars | The station's signed **BLE StationIdentity** artefact — the JSON object of [Chapter 06 §6.5.2](06-security.md) (`stationId`, `organizationId`, `stationPubKey`, `issuedAt`, `expiresAt`, `signatureAlgorithm`, `signature`), in OSPP Canonical Form. First delivered in the provisioning response; **re-issued** through ChangeConfiguration [MSG-013], which is the channel §6.5.2 names and relies on because `expiresAt` is short and the server re-issues before expiry. Write-only, mirroring `OfflinePassPublicKey` as §6.5.2 says — **not for confidentiality** (the station presents this artefact to any BLE peer during the handshake) but because a station's held identity is confirmed by completing a handshake, not by echoing 364 characters back through the configuration channel on every GetConfiguration. **Registered at 0.30.0.** It was named as a valid ChangeConfiguration key at two normative sites and was absent from this registry, so §8.2 rule 3 obliged a **conforming** station to answer `NotSupported` — and, because the batch is atomic, to apply nothing else in the same request. The specification mandated a rotation every conformant station had to refuse. |
...
| `MaxOfflineTransactions` | integer | `1000` | RW | Dynamic | 1000--10000 | Capacity of the station's TransactionEvent buffer, in events. The **floor is normative and is owned by** [Chapter 01 — Architecture](01-architecture.md), §6.5: TransactionEvent is a Category-1 message with a minimum capacity of 1000 and `MUST NOT` discard, and the §6.5 hardware table does **not** size its `MUST` storage level for that: 512 KB against a derived **~1.6 MB** for the Category-1 floors, a shortfall §6.5 records as OPEN rather than closes, because raising a mandatory storage level changes the bill of materials of every conformant station. **Size hardware to the derived figure, not to the level row.** This sentence asserted the opposite — that the level was sized for exactly this floor — until 0.30.0; the arithmetic disproving it has been in §6.5 since 0.25.0. A value below the floor cannot satisfy §6.5, which is why the range starts there rather than allowing it; the ceiling bounds the configuration value, not the hardware. Distinct from the pass-carried `constraints.stationMaxOfflineTx` ([`offline-pass.md` §2.2](profiles/offline/offline-pass.md)), which is a per-pass policy limit on starting further offline sessions, not a storage capacity. |
...
| `RevocationEpoch` | integer | `0` | RW | Dynamic | 0--2147483647 | OfflinePass revocation epoch for the tenant this station belongs to. Incremented by the server to batch-revoke that tenant's OfflinePasses issued before this epoch; the scope is normative and is owned by [Chapter 06 §6.6](06-security.md#66-epoch-based-revocation). The station holds exactly one value and does not model the scope. |
...
**Order: persist the accepted batch before applying any Dynamic key, and treat an incomplete batch as absent.** §8.2 rule 5 requires a Dynamic key to be applied **immediately**, and the paragraph above requires persistence **after** a successful ChangeConfiguration; nothing joined the two, so a power loss between them left the station running values it had not stored. The station **MUST** write the whole accepted batch durably first, then apply — write-then-swap, the same atomicity [`update-service-catalog.md` §6](profiles/device-management/update-service-catalog.md) already states for the catalog (*"there **MUST NOT** be a window where a partial catalog is active"*). On boot the station **MUST** discard a batch record that is incomplete rather than merge its written half with defaults for the rest.

**Why this is not housekeeping.** The array form exists for *correlated* keys, and this chapter's own worked example is the pair `OfflinePassPublicKey` + `RevocationEpoch` — both Dynamic, so both take rule 5's immediate-apply arm. Half-applied, the station holds the new signing key against the old epoch, or the new epoch against the old key; either way it accepts and refuses OfflinePasses wrongly, and **it cannot detect that it has done so** — [`offline-pass.schema.json`](../schemas/common/offline-pass.schema.json) is closed and carries no key identifier, so a pass verified under one key and judged against the other looks exactly like a valid one. Falling back to defaults is the worst branch available: `RevocationEpoch`'s default is `0`, the most permissive value in its range, so the failure mode is to accept revoked passes.
