...
| 0.16.0 | 2026-08-13 | OSPP Authors | **A configuration-cycle consistency pass, and the finding that matters most is a disproof.** A server-side note held that two conformance cases still taught a ChangeConfiguration response shape the schema forbids — a bare top-level `status`. They do not: `f872b23` repaired TC-DM-001 and TC-DM-006 on 2026-08-10, released in `0.12.0`, and both now assert the opposite explicitly. The note had gone stale and was carried forward unmeasured. **`Accepted` meant two incompatible things and one of them could revoke a signing key.** §8.2's status table and `change-configuration.md` §5 both defined it as *applied* / *set successfully*, while the atomicity **MUST** three chapters state makes it a per-key **validation verdict** that applies nothing if any sibling entry is `Rejected` or `NotSupported`. `06-security.md` §6.7 then counted a station as holding a rotated OfflinePass key *"when, and only when, it has returned `Accepted`"* — so a batch carrying one rejected key would mark the station rolled out, and the old signing key would be revoked out from under it. Both definitions are now batch-conditional, §6.7 carries the proviso, and a single-key batch is **RECOMMENDED**. **§8.4's example answered for a key the request never sent** (`FirmwareVersion` against a request carrying `OfflinePassPublicKey` and `RevocationEpoch`), breaking the one-entry-per-request-key **MUST** in the same file; §8.5 answered `RebootRequired` for `HeartbeatIntervalSeconds`, which is **Dynamic**, contradicting §8.2 rule 5 and §8.2's own example of the same key. **The registry states itself twice and nothing compared the two statements:** §§2--6 carry Range and Description, §9 carries an index and a profile label, and the two live gates each read only one of them. §9 is now declared derived, §§2--6 normative — the reading two obligations in the chapter already depend on — and `tools/check-config-ranges.py` compares them. That split is why §9 said `Device Mgmt` against §1.5's `Device Management`, and so why **neither SDK matched the spec on the profile label: there was no single spelling to match.** **The Range column is declared normative and given a grammar** — five forms, counts checked — and the new gate also compares restated ranges. It reports six open findings: `HeartbeatIntervalSeconds` is `30--3600` in the registry and `10--3600` at four other sites including its own schema, and `retryInterval` admits `1` against `BootRetryInterval`'s `10--600`. Both are quantities carried by a config key **and** a dedicated wire field, both disagreements decide what is legal on the wire, and both are recorded in KNOWN-ISSUES rather than picked. **MINOR is carried by §6.7's rollout obligation**; the rest are clarifications, example repairs and citation fixes. `protocolVersion` stays `0.3.0`; nothing changes on the wire. |
