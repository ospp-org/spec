...
3. **Token state.** The token authenticated, but has already been consumed and this request is not a replay of the provision that consumed it → `409 Conflict` / `4018 PROVISIONING_TOKEN_CONSUMED` ([Chapter 07 §3.4](07-errors.md)), with the discriminator in `details.reason`. Evaluated after authentication and before any key is examined, for the same reason: the token's state settles whether a certificate can be issued at all, and no key in the body can change it. A `4018` rejection issues no certificate and alters no binding.
