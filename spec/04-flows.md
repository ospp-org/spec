...
8. Server validates the token (not expired, not used), signs the CSR with the Station CA, and returns the provisioning response per [`provisioning-response.schema.json`](../schemas/provisioning-response.schema.json) — see schema for the canonical field set and constraints (`stationId`, `bays[]`, the deprecated `bayIds[]`, `clientCert`, `stationCaChain`, `brokerRootCa` (optional, broker server-cert trust anchor), `rootCaThumbprint` (optional, SHA-256 Root CA thumbprint for local trust-anchor pinning), `serverVerifyKey`, and the `mqttConfig` block: broker host/port/URI, client-ID template, topic prefix, QoS level, keep-alive, clean-start, session-expiry, TLS version, MQTT version, optional LWT topic). Defaults align with the normative MQTT connection parameters in [Chapter 02 — Transport §1.2](02-transport.md#12-connection-parameters)
9. SSP stores the response — issued certificate, `stationCaChain`, `brokerRootCa`, `rootCaThumbprint`, `serverVerifyKey`, `mqttConfig`, `stationId`, `bays` (and, until 0.12.0, `bayIds`) — in NVS alongside the keys already committed at step 6b, and marks itself as provisioned
...
**Why the pair is carried explicitly rather than by array position.** Through 0.10.0 this correspondence was carried by the **order** of `bayIds`: the element at index *i* was the bay whose `bayNumber` is *i + 1*. That encoding is expressible only for a **dense** bay set, and a station's bay set need not be dense — a station whose second bay was never fitted has bays `{1, 3}`, and a positional array would have to invent a bay 2 to reach index 2. It also made the mapping the one place in the whole protocol where array position was the sole carrier of a correspondence, and the only one required to survive a message, a reboot and a write to non-volatile storage. Everywhere else the protocol uses order it **also** echoes the identifier — `ChangeConfiguration` results follow request order *and* repeat `key` in every member — so explicit pairing is the convention, not the exception.

> **Deprecated (0.11.0).** `bayIds` is superseded by `bays` and will be **removed in 0.12.0**. It remains **required** for this one release, deliberately: the server emits it and a station built against 0.10.0 reads it, so dropping it in the release that introduces the replacement would break every deployed station. A server **MUST** emit both, and they **MUST** agree — `bayIds` **MUST** list exactly the `bayId` values in `bays`, ordered so that index *i* is the bay whose `bayNumber` is *i + 1*. A server that cannot satisfy that ordering because the station's bay set is **not dense MUST reject the provisioning request** rather than emit a `bayIds` array that misstates the mapping; a non-dense topology is therefore usable only once both parties have moved to `bays`. A station implementing 0.11.0 or later **MUST** read `bays` and **MUST** ignore `bayIds`. Servers **MUST NOT** reorder either field between the original response and a replay; that is part of why both are byte-identical above.
...
**Descriptive drift MUST be ignored.** A retry whose descriptive fields differ — `serialNumber`, the deprecated `bayCount`, and every program `label` in `bays` — is a replay. A `label` is descriptive for the same reason `serialNumber` is: it names the hardware for a human, and nothing in the protocol keys off it. This is what stops a corrected typo in a firmware constant from being read as a different station. The **structural** members of `bays` — the `bayNumber` set and each bay's `programNumber` set — are **not** descriptive: they are the topology itself, and drift in them is a mismatch, rejected at step 5 of *Error precedence* with `4020 BAY_COUNT_MISMATCH` rather than ignored here. These are the only descriptive fields the request carries: the body is a closed field set ([`provisioning-request.schema.json`](../schemas/provisioning-request.schema.json)), and station model and firmware version are reported in BootNotification ([Chapter 03 — Messages](03-messages.md)), not at provisioning. The server **MUST** return `200 OK` with the **byte-identical** certificate already issued and **MUST NOT** mint a second certificate. For these fields the token, not the body, determines the certificate. What the rest of the response carries is governed by *What a replay returns* below.
...
| `bayIds` | **deprecated**, see below; retained byte-identically for the deprecation window, **order included**, since a 0.10.0 station still reads that order |
