...
**A1 — Rejected:** Server returns `Rejected` with `retryInterval`. The SSP waits `retryInterval` seconds and retries from step 6. The causes are the codes [Chapter 07 §4.1](07-errors.md#41-station--server-mqtt-actions) assigns to this message and no others — principally `2001 STATION_NOT_REGISTERED`, which its own registry row extends to a tenant move.

> **Two causes this line used to name are not causes of a Boot `Rejected`, and each sends a reader to the wrong recovery.** A **revoked certificate** is refused at the connection, as `1004 CERTIFICATE_ERROR` — the code names *revoked* explicitly, the broker **MUST** verify the client certificate against the CA trust chain ([Chapter 02 §1.3](02-transport.md#13-tls-12-floor-13-recommended)) and, as of `0.27.0`, **MUST** establish that it has not been revoked ([Chapter 06 §2.1.1](06-security.md#211-revocation-checking)) — the second is the clause that actually makes this sentence true, and until it existed the chain obligation was the nearest thing to cite. `1004`'s recovery forbids entering provisioning mode or discarding credentials. A station refused there never sends a BootNotification, so it never receives this response; and no code in §4.1's BootNotification set names a revoked certificate, so a server that did answer here could not say why. A **decommissioned station** is not a thing this specification has: decommissioning and disposal are out of scope as of `0.25.0` ([Chapter 00 §1.3](00-introduction.md#13-out-of-scope)) — there is no terminal station state and no protocol event that ends a station's life. Both survived the `0.25.0` sweep that made the second of those decisions.
...
|-------|-------|------------|
| CONNACK refused | Invalid certificate, broker unreachable, or the broker cannot establish the certificate's revocation status and its grace has expired ([Chapter 06 §2.1.1](06-security.md#211-revocation-checking)) | Reconnect with exponential backoff (see [Flow §9](#9-error-recovery--reconnection)) |
| `1007 PROTOCOL_VERSION_MISMATCH` | The declared `protocolVersion` is not in the server's supported set — negotiation is exact match ([VERSIONING.md](../VERSIONING.md)) | Log error, record `supportedVersions`, and **keep retrying BootNotification** at `retryInterval` (default 30 s), per [Core profile CORE-011](profiles/core/README.md); stay in the `Rejected` restricted state meanwhile. The mismatch is fixed by new station firmware **or** by the server adding that version to its set — the retry is what recovers the second case without a site visit, and a rejected station accepts no commands, so it cannot be handed firmware while stopped |
...
sequenceDiagram
    ...
    Admin->>Server: Register station (model, vendor, bays[])
    Server-->>Admin: stationId, bays[], provisioningToken (TTL set at issuance)
...
2. Server generates `stationId` (`stn_{uuid}`), one `bayId` (`bay_{uuid}`) per registered bay together with the `bays[]` pairing that names each bay's `bayNumber`, and a provisioning token (UUID, single-use, with a TTL fixed at issuance)
...
**`bays` pairs each `bayId` with its `bayNumber` explicitly.** The response carries one object per bay, each naming both members: `{"bayId": "bay_…", "bayNumber": 3}`. Order is **not** significant — the pairing is carried by the fields — so a server **MAY** emit the entries in any order and a station **MUST NOT** infer anything from their sequence. There is exactly one entry per bay the station declared in its request, and the set of `bayNumber` values **MUST** equal the declared set exactly.

This is the **only** mapping the station is given, and it exists because the station is required to produce the pair. A station knows which bays it physically has and numbers them itself, but `bayId` values are **server-assigned** ([Chapter 01 — Architecture §3.2](01-architecture.md)) and arrive only here; nothing else in any profile relates one to a bay number. Yet the first message the station sends after `BootNotification` `Accepted` is one StatusNotification per bay carrying **both** `bayId` and `bayNumber` ([Core profile CORE-004](profiles/core/README.md), [StatusNotification §7](profiles/core/status-notification.md)) — so if the correspondence were not fixed here, it could not be established anywhere.

**Why the pair is carried explicitly rather than by array position.** A correspondence carried by array position — the element at index *i* being the bay whose `bayNumber` is *i + 1* — is definable only for a **dense** bay set, and a station's bay set need not be dense: a station whose second bay was never fitted has bays `{1, 3}`, and a positional array would have to invent a bay 2 to reach index 2. It would also be the one place in the whole protocol where array position was the sole carrier of a correspondence, and the only one required to survive a message, a reboot and a write to non-volatile storage. Everywhere else the protocol uses order it **also** echoes the identifier — `ChangeConfiguration` results follow request order *and* repeat `key` in every member — so explicit pairing is the convention, not the exception.

**A non-dense bay set is legal, everywhere.** Nothing in the provisioning request, the response or BootNotification requires bay numbers to be contiguous, and a server **MUST NOT** reject a declaration for being non-dense.
...
| BootNotification `Rejected` after reconnect | Read the `errorCode`: the causes are the set [§4.1](07-errors.md#41-station--server-mqtt-actions) assigns to BootNotification. Decommissioning is **not** among them — it is out of scope ([Chapter 00 §1.3](00-introduction.md#13-out-of-scope)) — and the SSP **MUST** keep retrying per CORE-011 rather than await intervention |
