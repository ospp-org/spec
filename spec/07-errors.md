...
| 3000–3999 | **Session & Bay Errors** | Application | 21 | Bay state, session lifecycle, reservation, and service errors |
...
**Total: 120 standard error codes.**
...
| 409 | 3001, **3003**, 3014, 3019, 3020, 4015, 6005, 6008 | Conflict — resource state conflict |
...
| 3003 | `SERVICE_UNAVAILABLE` | Warning | true | The requested service is declared on this bay and is not deliverable right now (the hardware that delivers it is faulted, it is disabled by configuration, or it is temporarily out of consumables). **This is availability, not existence**: a program that is not declared on the bay at all is `3019 SERVICE_NOT_BOUND`, never this code. These causes have different correct recoveries, so this is a **branching entry** per [§1.4](#14-provenance-of-errordescription-and-recommendedaction), and servers **MUST** carry `details.cause`: `station-reported` — the station reported this program `available: false` on a StatusNotification and the condition clears when it reports it available again, with no operator action; `disabled` — an operator or a tenant-lifecycle rule disabled it, and it clears only by operator action; `consumable` — a depleted consumable, which clears on refill. **Only the first is the station's statement about itself.** An absent discriminator needs no guess and **MUST** be read as `station-reported`, which is the only cause a station itself ever emits: on the wire to and from a station this code has always meant exactly one thing, and the ambiguity this discriminator resolves is on the server's REST surface, where one action answers `3003` both for a program the station reported dead and for a tenant-lifecycle refusal. **No code is added for this.** `1004` is the precedent — one condition, several causes, one `details.cause` — and a second registry entry would assert a distinction the station does not make. This cell opened with *"hardware not present"* until 0.30.0 — an existence formulation, contradicting the rest of its own text and the profile. **The unavailability is frequently at one program ordinal rather than across the bay** — a single refilled-tomorrow nozzle on a bay whose other programs run — and the ordinal is the fact that separates a consumable from a dead bay. **It is already on the wire and needs no new member:** `start-service-response.programNumber` is REQUIRED when `status` is `Rejected`, [`ble/start-service-response`](../schemas/ble/start-service-response.schema.json) carries the same echo, and on REST the ordinal travels as `details.programNumber`. Distinct from `3017 PROGRAM_NOT_DECLARED`, which is the ordinal naming nothing at all; this code presupposes the program **is** declared and is merely unavailable right now. **The ordinal is informational, and that is a statement about `details.programNumber`, not about this entry.** `details.cause` is the discriminator and is REQUIRED, per the branching obligation above; `details.programNumber` narrows the report to one program and is OPTIONAL ([§1.3](#13-error-object-fields)). Until 0.33.0 this sentence read *"This entry does not branch, and `details` stays OPTIONAL for it … §1.4's branching obligation is not engaged"*, which contradicted the obligation four sentences above it. It was the older of the two: it stood unchanged at `v0.29.0` and `v0.30.0`, and the `v0.31.0` edit that made this a branching entry did not remove it, so two releases shipped a cell that answered its own question both ways — on a code that rides `StartServiceResponse`, where a station answering `Rejected` could not tell whether `details.cause` was required of it. The branching half is the one that survives, because the recommendedAction beside it gives three different recoveries, which is what branching means. | **Branch on `details.cause`; absent means `station-reported`.** App: select a different service, or a different bay. Station and server: echo the refused `programNumber` — REQUIRED on a `Rejected` StartService response, `details.programNumber` on REST. `station-reported`: wait for the station to report the ordinal available again; nothing server-side changes. `disabled`: an operator must re-enable it. `consumable`: refill at that ordinal. |
...
| 3020 | `BINDING_UNCOVERED` | Error | true | The server holds a service→program binding for the requested (bay, service) pair, and the ordinal that binding names is **not** in the set the station declared for that bay — at provisioning or at its most recent boot. The binding exists, which is what separates this from `3019 SERVICE_NOT_BOUND`: `3019`'s entry asserts the server holds **no** binding and its action tells an operator to create one, which here is already there. The ordinal is declared nowhere on that bay, which separates it from `3003 SERVICE_UNAVAILABLE`, whose entry presupposes the program **is** declared and is merely undeliverable right now. It is a separate entry rather than a branch of `3019` for the reason `3019` is separate from `3017`: the two conditions are opposites — a binding **absent** and a binding **present but stale** — and a single code covering both would carry one of two instructions that is always wrong for the other. The condition arises without anyone acting on the binding: a station that declares a narrower program set at its next boot uncovers every binding naming an ordinal it dropped, and that declaration does not touch the binding. **Server-originated toward the requesting client** — an app, a dashboard, an operator API call — and **MUST NOT** be transmitted to a station, which declared correctly and has no part in the fault. A server **MUST NOT** substitute a neighbouring ordinal, **MUST NOT** clamp to the highest declared one, and **MUST NOT** dispatch StartService and leave the station to answer `3017 PROGRAM_NOT_DECLARED` — that refusal is a true statement about a message the server should not have sent. **Silence is not this code.** Where the server holds no declaration at all for the bay, the bay has told it nothing, and nothing is not a report that a program is gone: this entry does not apply and the ordinal is dispatched as before. `details` carries `bayId`, `serviceId`, `programNumber` and `declaredPrograms`. | Operator: the service→program binding on this bay names ordinal `details.programNumber`, which the station no longer declares. Re-bind the service to one of `details.declaredPrograms`, or re-provision the station if its hardware changed. Server: do not dispatch StartService, and do not substitute a neighbouring ordinal. The customer has not been charged, because nothing was started. |
...
| 4002 | `OFFLINE_LIMIT_EXCEEDED` | Error | false | An offline spending or capacity ceiling has been reached and the transaction is refused. Two families of ceiling share this code. **Pass-scoped:** the OfflinePass's `maxTotalCredits` or `maxUses` is reached ([`06-security.md` §6.1.1](06-security.md) checks #6 and #7) — no further offline transactions are permitted with **this pass**. **Station-scoped:** the station's own `stationOfflineWindowHours` or `stationMaxOfflineTx` is reached ([`profiles/offline/offline-pass.md` §2.2](profiles/offline/offline-pass.md)) — the station refuses **every** pass until it has synchronised, and the two are distinguished by `details.constraint` rather than by a second code. | App: the user must go online to request a new OfflinePass (or top up credits). Where `details.constraint` names a station-scoped ceiling, a different pass will not help and the station itself must reconnect. |
...
| Code | errorText | Severity | Recoverable | Description | Recommended Action |
|:----:|-----------|:--------:|:-----------:|-------------|-------------------|
| 4010 | `CSR_INVALID` | Error | true | The Certificate Signing Request is malformed, uses a prohibited key algorithm (must be ECDSA P-256), has an invalid Subject CN, or its `SubjectPublicKeyInfo` cannot be decoded. Reachable from two paths whose safe recovery is **opposite**: certificate renewal (SignCertificate [MSG-022]), where a fresh keypair *is* the renewal, and `POST /api/v1/stations/provision`, where after the token has issued a certificate the submitted key is **bound** and regenerating it is answered `4015` forever. Servers **MUST** therefore carry `details.phase` (`first-provision`, `retry`, or `renewal`) on this code **wherever the response can hold it**, which is the REST provisioning route (Appendix C). On **SignCertificate [MSG-022]** it cannot: that response schema is closed and carries no `details` member. No discriminator is needed there and none is transmitted — SignCertificate is the renewal flow and nothing else ([`certificate-renewal.md`](profiles/security/certificate-renewal.md)), so `4010` arriving on it is `renewal` by transport, and a station **MUST** read it that way rather than applying the absent-discriminator default below, which is written for the provisioning endpoint and would tell it not to regenerate the very keypair the renewal consists of. At the provisioning endpoint: HTTP `400 Bad Request`, evaluated **fourth** — after `4017`, `2019` and `4018`, and before `4020`, `4019`, `4016` and `4015` — see *Error precedence* in [Flows §2](04-flows.md#single-use-and-idempotent-retry); the rejection does not consume the token and alters no binding. | Station: branch on `details.phase`. `first-provision` or `renewal` — regenerate the keypair and CSR correctly and resubmit; nothing is bound yet. `retry` — do NOT regenerate: a fresh key is answered `4015`, not recoverable. Resubmit a well-formed CSR over the bound key, or request a new token. Absent on REST means `retry`; on SignCertificate [MSG-022] it is always absent and means `renewal`. Server: log the validation failure. |
...
| 5016 | `VERSION_ALREADY_INSTALLED` | Warning | false | The requested firmware version is **the same as or older than** the one the station is running, and `forceDowngrade` was not set. The two conditions share this code deliberately — see [Chapter 06 §4.6.1](06-security.md#461-anti-downgrade-protection), where the predicate is stated once. Versions compare as three integers, numerically. | No action required. Server: update its records to reflect the station's current firmware version. |
...
| 5113 | `OUTCOME_INDETERMINATE` | Warning | false | The station cannot determine what a bay delivered. It holds a durable pre-effect record ([`start-service.md` §6](profiles/transaction/start-service.md) rule 9) proving a service was energised, and no completion record, and the interruption destroyed the evidence either way — the reboot case of [`05-state-machines.md` §3.5](05-state-machines.md#35-per-session-sequence-number-seqno-and-crash-resilience) rule 6. **This is the one 5xxx code that asserts no fault was detected**, which is why none of the others is true of it: every other member of this range names something the station observed, and here the station observed nothing and says so. It is the bay-level answer to the case [`status-notification.md` §6.1](profiles/core/status-notification.md) rule 3 already licences at program level — *report without guessing a cause* — which CORE-012 does not extend to a bay. | Station: report the bay `Faulted` and emit the SecurityEvent [MSG-012] that carries the `sessionId`. Server: settle on the estimate, and record that the closing figure is unmeasured rather than observed. Operator: inspect the bay before returning it to service. |
...
| Heartbeat [MSG-008] | *(none — a Heartbeat is never answered with an error. See the note below.)* |
...
| TransactionEvent [MSG-007] | **2002**, **2003**, **2004**, **2005**, **2006**, **2014**, **2015**, **2016**, **2017**, 1005, 3015, 6001 — **recorded, not transmitted** ([`reconciliation.md` §6.4](profiles/offline/reconciliation.md#64-response)) |
| AuthorizeOfflinePass [MSG-002] | **2002**, **2003**, **2004**, **2005**, **2006**, **2015**, 1005, 4002, 4003, 4004, 6001 — **recorded, not transmitted** ([`authorize-offline-pass.md` §7](profiles/offline/authorize-offline-pass.md#7-error-codes)) |
...
> **Heartbeat is a REQUEST whose RESPONSE cannot say no, and the resolution is that it never needs to.**
> [`heartbeat-response.schema.json`](../schemas/mqtt/heartbeat-response.schema.json) is `{serverTime}`, REQUIRED, `additionalProperties: false` — no `errorCode`, no `errorText`, and no `status` either, so a rejection is not merely uncoded but unexpressible ([§2.1](#21-mqtt-error-response)). A server **MUST NOT** answer a Heartbeat with anything but `serverTime`, and **MUST NOT** invent a status member to carry a refusal.
>
> This row listed `1005`, `1010`, `5106` and `6001` until `0.26.0`. **Not one of them was ever a value a Heartbeat RESPONSE carries**, and each already has a home that works:
>
> - **`1010 MESSAGE_TIMEOUT`** is raised by the **station** when no response arrives. A response carrying it would disprove it. Its home is [Appendix B](#appendix-b--timeout-quick-reference), which already gives it on the Heartbeat row.
> - **`5106 CLOCK_ERROR`** is a **station-side** condition detected by comparing `serverTime` against the local clock. Its own registry row says so, and [Chapter 03 §5.1](03-messages.md) has the station log it and **SHOULD** report it as a SecurityEvent [MSG-012] with `type: ClockSkew`. It is diagnosed **from** a successful Heartbeat response, never carried **in** one.
> - **`1005 INVALID_MESSAGE_FORMAT`** on a malformed Heartbeat REQUEST: its own *Recommended Action* is *"Log the malformed message"*, not *respond*, which is what [Chapter 02 §11](02-transport.md#11-error-scenarios-summary) already prescribes for an unreadable message on any action.
> - **`6001 SERVER_INTERNAL_ERROR`** is the only one a station could have acted on, and its recovery — *retry with exponential backoff* — is what a station does anyway when a Heartbeat goes unanswered. Nothing is lost by the silence, because the silence produces the same behaviour the code would have asked for.
>
> **The profile already said this; the chapter was the outlier.** [`heartbeat.md` §8](profiles/core/heartbeat.md#8-error-handling) gives all four conditions with their real behaviours — *"Server drops the message"*, *"Server **MAY** omit the response"*, *"Station logs warning and continues"*, *"Station logs warning and adjusts clock from response"* — and not one of them is a response the station reads a code out of. This row is being reconciled to the profile, not decided against it.
>
...
|----------|:----:|---------------------|
| `POST /sessions/start` | 400, 402, 404, 409, 422, 500, 504 | 3000, 3001, 3002, 3003, 3004, 3005, 3008, 3009, 3010, 3011, 3012, 3013, 3014, 3019, 3020, 4001, 5000–5009, 5111, 6002, 6003, 6005, 6008 |
...
Each MQTT command (Server → Station) listed below has a specific retry policy based on urgency and
idempotency. **Four are deliberately absent** and are single-attempt with no retry:
`CertificateInstall` [MSG-023], `TriggerCertificateRenewal` [MSG-024], `DataTransfer` [MSG-025] and
`TriggerMessage` [MSG-026]. Each carries its own timeout in
[Chapter 03](03-messages.md#mqtt-messages-27-actions), and re-issuing any of them is an
operator or server decision rather than a transport-level retry — so there is no delay schedule to
state. They are named here because "Each MQTT command" over a table that held eleven of fifteen
reads as a complete enumeration.
...
| 3020 | `BINDING_UNCOVERED` | Error | S |
...
| 5113 | `OUTCOME_INDETERMINATE` | Warning | H |
...
This table consolidates the timeout values from the protocol for implementer reference, **plus** the
non-message deadlines (payment, BLE, reconnect) that have no row in
[Chapter 03](03-messages.md#mqtt-messages-27-actions). It is **not** the complete set of
message timeouts and **MUST NOT** be read as one: Chapter 03's master table is where every action's
timeout is declared, and the eight actions below are declared there and deliberately not repeated
here — `AuthorizeOfflinePass` (15s), `CancelReservation` (5s), `TransactionEvent` (60s),
`SignCertificate` (30s), `CertificateInstall` (30s), `TriggerCertificateRenewal` (10s),
`DataTransfer` (30s) and `TriggerMessage` (10s). Naming them is what keeps "consolidates" from
reading as "is exhaustive", which is how this table was read while eight rows were missing from it.
