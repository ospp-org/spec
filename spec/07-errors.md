...
| 4016 | `PROVISIONING_KEY_REUSE` | Error | true | The provisioning request submitted the **same public key** as both the `tlsCsr` subject key and the `receiptSigningPublicKey`. The station's mTLS client key and receipt-signing key **MUST** be distinct key pairs ([Chapter 06 §4.3](06-security.md)), so that receipt verification survives rotation or revocation of the TLS credential. HTTP `422 Unprocessable Entity`. Evaluated **after** token validity (`2019`) and **before** the bound-set comparison (`4015`) — see *Error precedence* in [Flows §2](04-flows.md#single-use-and-idempotent-retry). Distinct from `4015 PROVISIONING_KEY_MISMATCH`: this is a non-conformant request at **first** provision, not a changed identity on a retry. | Station: generate a **separate** ECDSA P-256 key pair for receipt signing and resubmit. The token is **not** consumed by this rejection, so the same token may be reused once the keys are corrected. Firmware that derives both roles from one key slot must be updated. |
