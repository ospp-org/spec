...
| 2019 | `PROVISIONING_TOKEN_INVALID` | Error | false | The provisioning token presented to `POST /api/v1/stations/provision` is not usable: it has passed its TTL, has been **superseded** by a re-issuance for the same station, or has been administratively **revoked**. All three are terminal for that token. Servers **SHOULD** carry the discriminator in `details.reason` (`expired`, `superseded`, `revoked`). HTTP `401 Unauthorized`. Evaluated **first** of the three provisioning errors — see *Error precedence* in [Flows §2](04-flows.md#single-use-and-idempotent-retry). Distinct from `4015 PROVISIONING_KEY_MISMATCH`, where the token is valid but the submitted identity changed. See [Flows §2](04-flows.md#single-use-and-idempotent-retry). | Station: display the error and **await a new provisioning token** — no retry with this token can succeed. Operator: issue a fresh token. Do not regenerate keys in response to this error; the keys are not what was rejected. |
...
| 4015 | `PROVISIONING_KEY_MISMATCH` | Error | false | A retry of an already-consumed provisioning token did not match the **bound set** — the key kinds submitted at first provision and the key each carried. Either a bound key kind carried a **different public key**, or the **set** of key kinds differed (one added, or one dropped). The token authorises exactly one provisioned identity, so this is not a replay and no second certificate is issued. See [Flows §2 — Single-use and idempotent retry](04-flows.md#single-use-and-idempotent-retry). HTTP `409 Conflict`. Evaluated **last** of the three provisioning errors, after token validity (`2019`) and request self-consistency (`4016`) — see *Error precedence* there. | Station: **do NOT retry with this token** — no retry can succeed, because the token is permanently bound to the earlier key. Request a **new** provisioning token from the operator, then provision again with the keys currently held. Server: log the mismatch; the already-issued certificate is unaffected. |
| 4016 | `PROVISIONING_KEY_REUSE` | Error | true | The provisioning request submitted the **same public key** for two different roles. The keys a request submits **MUST** be pairwise distinct, and all three pairs are covered: `tlsCsr` subject key / `receiptSigningPublicKey`, `tlsCsr` subject key / `stationPubKey`, and `receiptSigningPublicKey` / `stationPubKey` ([Chapter 06 §4.3](06-security.md), [§6.5.2](06-security.md)). Separating the two ECDSA keys keeps receipt verification alive across rotation or revocation of the TLS credential; separating either from the BLE ECDH key is the stronger signing-vs-key-agreement rule (NIST SP 800-56A). Servers **SHOULD** name the colliding pair in `details`. HTTP `422 Unprocessable Entity`. Evaluated **after** token validity (`2019`) and **before** the bound-set comparison (`4015`) — see *Error precedence* in [Flows §2](04-flows.md#single-use-and-idempotent-retry). Distinct from `4015 PROVISIONING_KEY_MISMATCH`: this is a non-conformant request at **first** provision, not a changed identity on a retry. | Station: generate a **separate** key pair for the colliding role and resubmit. The token is **not** consumed by this rejection, so the same token may be reused once the keys are corrected. Firmware that derives both roles from one key slot must be updated. |
