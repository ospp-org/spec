...
> **Still outstanding:** `sign-inline-md.mjs`, `sign-example.mjs`, `verify-example-signatures.mjs`, `verify-ble-crypto.mjs`, `generate-ble-vectors.mjs` and `generate-tamper-vectors.mjs` do import `canonicalize` from `@ospp/protocol`, so they do not use the implementation written from the text. That is **six** of the twelve tools here, not the five this note carried: the roster was written on 2026-08-12 and `generate-tamper-vectors.mjs` landed on 2026-09-02, after it. **The dependency half is closed at 0.30.0:** this note said the installed copy was `0.5.4` when it was `0.13.0` — identical to the declared `^0.13.0`, so the recorded remedy was a no-op — and the pin is now `^0.40.0`, whose canonicalizer sorts by UTF-8 bytes. Re-measured on each bump: signatures green, zero signer drift. Re-pointing the six tools at [`canonical-form.mjs`](canonical-form.mjs) is still tracked in KNOWN-ISSUES.
