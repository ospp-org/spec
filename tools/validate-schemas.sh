...
# schema was checked. A gate must not be able to die of a dependency it never loads.
AJV="${AJV_BIN:-}"
if [ -z "$AJV" ]; then
...
if [ -z "$AJV" ] || ! "$AJV" help >/dev/null 2>&1; then
  ...
  echo "  then:          AJV_BIN=/tmp/ajv/node_modules/.bin/ajv ./tools/validate-schemas.sh" >&2
  exit 2
fi
