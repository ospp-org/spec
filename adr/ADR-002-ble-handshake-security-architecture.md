...
A Noise Protocol Framework instantiation was evaluated and rejected for OSPP's constraints: there was **no usable Noise library for P-256** on any platform then under consideration (PHP: none; ESP32/mbedTLS firmware: none with a P-256 backend — decisive, since the weakest link commands the design; React Native: none combining the NK/NX pattern *with* P-256). P-256 is outside the Noise spec (§12 permits only 25519/448). Because the handshake must be **byte-identical** across the app (TS) and the firmware (MCU), and both would hand-roll the framework anyway, Noise's "formal, hard to get wrong" benefit erodes exactly where bugs are most expensive (RF flash/debug cycles). FIDO's caBLEv2 (`Noise_NK_P256_AESGCM_SHA256`) is real but FIDO itself notes most off-the-shelf Noise libraries are unusable and implemented it manually.

> **Editorial note, 2026-08-06.** This paragraph previously named three implementations the handshake had to be byte-identical across: "the app (TS), the station simulator (PHP), and the firmware (MCU)". The PHP station simulator has been retired, so that sentence named a normative third implementation that no longer exists. It has been corrected to the two implementations that do: the app and the firmware. The PHP entry is retained in the library survey above because it records what was evaluated **at the time of the decision** — the rejection of Noise rested partly on it, and deleting it would misrepresent why this ADR concluded as it did. The decision itself is unchanged. Verified when correcting: neither simulator implements the handshake — the PHP one never carried any of its primitives.
