...
Four contradictions are real, verified, and **left open**, because closing them is a product decision about behaviour or money rather than a reconciliation of drift. Each is recorded here rather than resolved by picking the older sentence:

- **The override's domain on a connection-loss close.** `connection-lost.md` §5 is a **MUST** applying the low-delivery full refund when the grace timer expires without reconnection; `04-flows.md` §6 scopes the override to a SessionEnded `reason` of `Fault`, and a grace-expiry close produces no SessionEnded at all. `04-flows.md`'s own matrix bills that scenario pro-rata with no override. Both are normative, one is a MUST, and the answer changes what a customer is refunded. The literal value at that site is left hardcoded rather than renamed, so the open question stays visible instead of being half-tidied.
- **Duplicate ReserveBay carrying the same `reservationId`.** `03-messages.md` §3.1's idempotency row (*"same `reservationId` MUST return same result"*), `03-messages.md` §3.1's error row and `07-errors.md` §3.3 all scope `3014` to *another* session or user; `reserve-bay.md` §6 twice and `profiles/transaction/README.md` §4.3 (under an **Idempotency** heading, on the word *"duplicate"*) require `3014` for any request on a `Reserved` bay. Four sites each way, no precedence rule between chapter and profile, and either answer changes station firmware. Note the report's aggravating claim — that an idempotent retry would cancel a `Confirmed` reservation via `05-state-machines.md` §4.3 — is **false**: that row's From state is `Pending`, and `07-errors.md` §6 sets ReserveBay's Max Attempts to 1, so no spec-sanctioned retry exists.
- **`SessionTimeout` has no way to be reported.** `08-configuration.md` §3 lets a station stop an idle session (**MAY**, keyed on user interaction); `05-state-machines.md` §3.4 states it as a transition, keyed additionally on MeterValues. If the station does stop, it terminated without a StopService, so `session-ended.md` §6 requires a SessionEnded — but the `reason` enum is closed at six members and none is an inactivity timeout. Adding one is a wire change. The interval interaction breaks in both directions: at the default pair (60, 120) the MeterValues-counting trigger can never fire; at the legal edge (3600, 600) every session expires.
- **Chapter 04's phantom session-state vocabulary.** `pending_ack`, `idle`, `reserving` and `reserved` appear in `04-flows.md` Appendix C and propagate into the guide and six worked examples; none exists in any state machine, and `Authorized` is missing. The equivalent discipline was already applied to the bay table directly above it. Whether Chapter 04 aligns to Chapter 05 or declares this server-internal vocabulary is a documentation-architecture choice, not a drift repair.
