...
**MINOR is carried by one obligation change** — the OfflinePass key-rotation rollout test in `06-security.md` §6.7. Everything else is a clarification, an example repair, or a citation fix. `protocolVersion` stays `0.3.0`; nothing changes on the wire.
...
- **tools:** `check-config-ranges.py`, wired into `check-drift.yml`, making four drift checks. It compares §9 against §§2--6 on the key set and all four shared columns; validates every Range cell against §1.6's grammar and recomputes §1.6's counts; compares restated ranges elsewhere in `spec/`, `guides/` and `conformance/` against the registry; and compares a registry range against the JSON Schema bounds of the wire field carrying the same quantity. Measured: 16 restatement sites, 4 flagged, **4 real**, plus 2 schema comparisons, **both real**. RED-tested once per check. `BASELINE = 6`, a ratchet on the six open findings below.

- **docs:** two KNOWN-ISSUES entries, both recorded rather than decided because each decides what values are legal on the wire. **(1)** `HeartbeatIntervalSeconds` is `30--3600` in the registry and `10--3600` at four other sites — including `boot-notification-response.schema.json`, which is authoritative for the boot branch under `00-introduction.md` §3.5 — and `heartbeat.md:35` requires an out-of-range value to be **clamped** where §8.2 rule 4 requires it **rejected**, so the two disagree on the disposition as well as the bound; `connection-lost.md` publishes the derived staleness window as `35--12600s`, which is 3.5× a **10**-second floor. `BootRetryInterval` is `10--600` while `retryInterval` admits `1` and has no maximum, with no precedence rule at all. **(2)** The Device Management profile is **Required: Yes** in `08-configuration.md` §1.5 and *"RECOMMENDED but OPTIONAL"* in its own README — one name, two requirement levels, complicated by the README governing nine actions where §1.5 governs four keys.
