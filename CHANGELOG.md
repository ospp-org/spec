...
- `ServerSignedAuthReplay` is named as a SecurityEvent `type` by error `2018` (`07-errors.md` §3.2) but is **not yet present in the SecurityEvent enum** (`security-event.md` §4) — a pre-existing dangling reference. Closing it is coupled with the ServerSignedAuth / Partial-A work (N3) and stays with **S2**.
