...
- Counter-replay `eventId` single-ordinal alignment (`check_5 → check_10`) — the authorize-time §5 list is not yet reordered to a single shared ordinal with the reconcile gate (org binding is unified by `errorCode 2015` + cross-reference, not by index); the positional reorder is coupled to this `eventId` migration and stays with **S2**.
- `ServerSignedAuthReplay` is named as a SecurityEvent `type` by error `2018` (`07-errors.md` §3.2) but is **not yet present in the SecurityEvent enum** (`security-event.md` §4) — a pre-existing dangling reference. Closing it is coupled with the ServerSignedAuth / Partial-A work (N3) and stays with **S2**. (The replay itself is *rejected* by the §4.2.2 `appNonce` check + the AEAD channel; only the SecurityEvent *logging* is incomplete.)
...
#### S2-corrections (adversarial review)

An independent hostile review of S2 found 2 wire-contract blockers + 9 incompleteness/financial gaps + 4 minors — the green suite was pass-form-only and validated no SecurityEvent, so it missed them. All fixed on `main` before push:
