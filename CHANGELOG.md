...
Three questions this pass deliberately left open were answered by the specification's owners, and the reasoning is recorded because in each case the losing sentence was the one with the stronger RFC 2119 keyword.
...
Three of the four contradictions this pass left open have since been decided; the decisions and their reasoning are under **Decided** above. **One remains open** — duplicate ReserveBay — and is stated below with its full option space rather than resolved by picking the older sentence:

- **Duplicate ReserveBay carrying the same `reservationId`.** Scoped rather than resolved, and the scoping corrected three claims this entry previously made. **The eight sites are not four-versus-four**: only two of them key on identifier identity at all. `03-messages.md` §3.1's idempotency row (*"same `reservationId` MUST return same result"*) and its lead sentence (*"reject **any other** … ReserveBay commands"*) decide the case one way; `reserve-bay.md` §6 rule 2 (bay-state validation, no identifier test anywhere in it) and `profiles/transaction/README.md` §4.3 decide it the other. The remaining four — `03-messages.md` §3.1's error row, `07-errors.md` §3.3, `reserve-bay.md` §5.1 rule 6 and §7's table row — scope what `3014` *means* (a **different** holder) or are silent on whose reservation it is, and do not decide the question. `reserve-bay.md` §6 rule 2 is the load-bearing site: it is the only one unconditional on identity, and it is written as a mandatory validation step in a processing sequence, which is the form firmware implements literally. A ninth site nobody has counted, `05-state-machines.md` §2.3 (*"Bay has no active session or **existing reservation**"*), sits in the table that self-declares as canonical and turns on a word — *existing* — that nothing defines.

  **"Duplicate" is three situations, and only one of them is a duplicate.** With the first reservation **live**, the reservation is `Confirmed` and the bay `Reserved`: both answers are implementable and neither corrupts state. With it **expired**, the station has already released the bay to `Available` (§5.1 rule 4, **MUST**) — so the bay-state rule's precondition is unmet and *that* side does not reject either; it accepts a **new** reservation under a dead identifier. With it **consumed by a StartService**, the bay is `Occupied`, and `reserve-bay.md` §6 rule 2 branches to `3001 BAY_BUSY` **before** reaching its `Reserved` clause — so in the third situation **neither** side produces `3014`, which is the code the whole dispute is framed around. The second and third are not duplicate-handling questions but identifier-reuse-after-death questions, which no cited text asks.

  **The corpus has already answered this shape once, on the sibling message, and denied ReserveBay the means to copy it.** `03-messages.md` §3.2's CancelReservation idempotency row states two arms in one cell — already-cancelled returns `Accepted`, expired returns `3013 RESERVATION_EXPIRED` — and `cancel-reservation.md` §5 rules 2, 3 and 6 plus the `3012` registry entry complete a four-way partition (live / already-cancelled / expired / consumed-or-never-existed). ReserveBay's idempotency row carries no arms at all, and `3012` and `3013` are absent from **all three** of its error tables (Chapter 03, Chapter 07 §4, and its own §7). It has no conformant way to say *that reservation is expired* or *that reservation is gone*.

  **And the retention rule that would make an identifier-keyed answer implementable does not exist.** `02-transport.md` §5.3's horizon applies to *"Records of **sessions** that have reached a terminal state"*; the word *reservation* does not occur in the section. Every other identifier in OSPP carrying an idempotency obligation has an explicit retention rule — `messageId` 1 hour, terminal sessions 24 h, `offlineTxId` 30 days, REST `Idempotency-Key` 24 h, provisioning key bindings the token's full TTL, BLE receipts 10 minutes — and `reservationId` has none. `04-flows.md` §2 states the criterion in as many words for the provisioning case: retention is *"what makes the rules above executable … the comparison has nothing to compare against once the binding is discarded."* **This gap is already live independently of the decision**: `cancel-reservation.md` §5 rule 3 and `TC-TX-002` both require a station to answer `3013` about an expired `reservationId`, with no rule obliging it to have kept one.

  **Three corrections to this entry as first written.** *"`reserve-bay.md` §6 twice"* was wrong — the two rules are in different sections (§5.1 rule 6 and §6 rule 2) and do not say the same thing; the §5.1 one says *"new"* reservation requests, which admits a reading that excludes a same-identifier re-send. *"`07-errors.md` §6"* was the wrong section for the retry table: §6 is **Circuit Breaker**, and Max Attempts = 1 is in §5.3. And calling the aggravating claim **false** was too strong: it is false only if a re-sent ReserveBay does not return a server-side `Confirmed` reservation to `Pending`, and §4.3 contains no row either way — there is no `Confirmed → Pending` edge and no row modelling a re-send at all. Unresolvable from the text as it stands.
