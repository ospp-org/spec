...
### Five MQTT response schemas cannot carry an `errorCode`

`07-errors.md` §2.1 requires a rejection to carry `status`, `errorCode` and `errorText`, and §4 assigns error codes per action — but five response schemas declare no top-level `errorCode`/`errorText` and are closed: `transaction-event`, `authorize-offline-pass`, `heartbeat`, `change-configuration`, `data-transfer`. §2.1 names them and says how a rejection is signalled on each instead. The denominator is 20 response schemas under `schemas/mqtt/`; 15 declare one.

**This section read "Seven" until `0.35.0` and named eight.** Three have been fixed and each is recorded below rather than deleted. `boot-notification-response` was the eighth and went first — it carries four codes with four different recoveries on the path every station traverses at every boot, so the branching was unexecutable; it now declares `errorCode` and `errorText`, required when `status` is `Rejected`. `trigger-message-response` (`0.34.0`) and `get-configuration-response` (`0.35.0`) followed, and they were one finding measured twice: of the **14** Server → Station REQUEST actions, exactly two had a response that could not carry any of the three codes the *Implicit Error Codes* note makes implicit for all of them.

Of the five remaining, most are not defects:
...
- **Acceptable, noted.** `data-transfer-response` discriminates coarsely by `status` alone (`Rejected` / `UnknownVendor` / `UnknownData`). The status values are themselves meaningful and the recoveries do not diverge the way boot's four do, so the coarse signal is adequate. `trigger-message-response` sat in this bullet on the same reasoning and it did **not** survive contact with the implicit-codes note: `NotImplemented` answers a question about the *argument*, and none of the three implicit codes, which are about the *action*, had anywhere to go. Coarse-but-meaningful was true of the status values and false of the message.
...
- **Taken at `0.35.0`.** `get-configuration-response` declared no `status` at all, so a rejection was **not expressible** — a station could not refuse a GetConfiguration in any way the schema admitted. It now carries `errorCode`/`errorText`, paired, with both arrays empty on that branch. The part worth keeping is why it cost less than this entry assumed: the shape appeared to require relaxing `configuration` out of `required`, and it did not, because `configuration: []` was already the mandated answer when every requested key is unknown. The relaxation was measured and rejected — it turns an existing invalid conformance vector valid, and buys nothing the empty array does not already say.
